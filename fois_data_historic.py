...
# ---------------------- Data Fetching Logic ----------------------
def fetch_fois_data(client, config, date_str, spreadsheet, api_call_count):
    """
    Fetch data from Indian Railway FOIS API for a specific date, apply zone filter, and push to Google Sheets.
    Only fetch and append if the date is not already present in the sheet.
    Returns updated API call count.
    """
    df_var_name = f"df_{config['table_name']}"
    print(f"Fetching data for {df_var_name} with date {date_str}")
    # Skip if data for this date is already present in Google Sheet
    ...
    try:
        ...
        try:
            ...
            if isinstance(data, list) and data and isinstance(data[0], dict):
                ...
                if zone_column in df.columns:
                    ...
                    if df.empty:
                        print(f"\n{df_var_name} =")
                        print(f"No data found for {zone_column}='WR' on {date_str}")
                        return api_call_count
        ...
        print(f"Error making request for {df_var_name} on {date_str}: {e}")
        if hasattr(e, 'response') and e.response is not None:
            print(f"Status code: {e.response.status_code}")
            ...
            if e.response.status_code == 401:
                ...
                try:
                    ...
                    print(f"Retry failed for {df_var_name} on {date_str}: {retry_error}")
        return api_call_count
...
def parse_arguments():
    """
    Parse command-line arguments to select specific API endpoint and number of days.
    """
    parser = argparse.ArgumentParser(description='Fetch historic Indian Railway FOIS API data.')
    parser.add_argument('--days', type=int, required=True,
                        help='Number of days to fetch data for, starting from yesterday.')
...
def main():
    """
    Main function to execute the FOIS API calls for multiple days and push data to Google Sheets.
    """
    try:
        ...
        # Counter for API calls to trigger sleep after 10 calls
        api_call_count = 0

        # Fetch data for each day, starting from yesterday
        for day_offset in range(1, args.days + 1):
            ...
            for config in api_configs_to_run:
                ...
                print(f"\nProcessing endpoint: {endpoint_name} for date {date_str}")
                api_call_count = fetch_fois_data(client, config, date_str, spreadsheet, api_call_count)
                
                # Delay to avoid Google Sheets API rate limits
                time.sleep(1)

                # Sleep for 60 seconds after every 10 API calls
                if api_call_count > 0 and api_call_count % 10 == 0:
                    print(f"Reached {api_call_count} API calls. Sleeping for 60 seconds to avoid rate limits...")
                    time.sleep(60)
        
        client.revoke_token()
        print(f"\nCompleted processing. Total API calls made: {api_call_count}")
        sys.exit(0)
